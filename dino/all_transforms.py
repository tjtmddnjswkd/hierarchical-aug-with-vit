...
__all__ = ["Compose", "ToTensor", "PILToTensor", "ConvertImageDtype", "ToPILImage", "Normalize", "Resize", "Scale",
           ...
           "RandomHorizontalFlip", "RandomVerticalFlip", "RandomResizedCrop", "My_RandomResizedCrop", "RandomSizedCrop", "FiveCrop", "TenCrop",
           "LinearTransformation", "ColorJitter", "My_ColorJitter", "RandomRotation", "RandomAffine", "Grayscale", "RandomGrayscale",
...
class RandomApply(torch.nn.Module):
    ...
    def __init__(self, transforms, p=0.5, param=False):
        super().__init__()
        self.transforms = transforms
        self.p = p
        self.param = param
...
class My_RandomResizedCrop(torch.nn.Module):
    """Crop a random portion of image and resize it to a given size.
...
class My_ColorJitter(torch.nn.Module):
    """Randomly change the brightness, contrast, saturation and hue of an image.
    If the image is torch Tensor, it is expected
    to have [..., 3, H, W] shape, where ... means an arbitrary number of leading dimensions.
    If img is PIL Image, mode "1", "L", "I", "F" and modes with transparency (alpha channel) are not supported.
